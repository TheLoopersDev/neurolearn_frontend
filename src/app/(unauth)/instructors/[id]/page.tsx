...
// <<-- SỬA Ở ĐÂY: Hàm này giờ nhận instructorId (string) thay vì cả object User -->>
...
const InstructorDetailPage: React.FC<InstructorDetailPageProps> = ({ params }) => {
  const { id } = params;
  ...
  // Chỉ cần MỘT useEffect để fetch tất cả dữ liệu
  useEffect(() => {
    if (id) {
      setIsLoading(true);
      const timer = setTimeout(() => {
        ...
        if (instructorData) {
          ...
          // <<-- SỬA Ở ĐÂY: Truyền `id` thay vì cả object -->>
