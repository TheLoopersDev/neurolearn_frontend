...
export async function POST(req: NextRequest) {
  try {
    ...
    if (mode === 'curriculum') {
      ...
      try {
        ...
        if (file) {
          ...
          const isDocx = fileType === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' || lowerName.endsWith('.docx');
      ...
      if (!curriculum || !Array.isArray(curriculum.sections)) {
        return NextResponse.json({ error: 'Invalid curriculum JSON from Gemini', raw }, { status: 502 });
      }
    ...
    if (mode === 'quiz') {
      ...
      const dist = questionConfigs
        .map((c) => `${c.type}:${Number(c.count) || 0}`)
        .join(', ');
      ...
      if (!Array.isArray(questions)) {
        return NextResponse.json({ error: 'Invalid questions JSON from Gemini', raw }, { status: 502 });
      }

      // Light validation/normalization to fit UI
      const normalized = questions.map((q: any, idx: number) => {
        const id = String(q?.id ?? `q${idx + 1}`);
        const options = Array.isArray(q?.options) ? q.options.map((o: any, oi: number) => ({ id: String(o?.id ?? `o${oi + 1}`), text: String(o?.text ?? `Option ${oi + 1}`) })) : [];
        const correctAnswerIds = Array.isArray(q?.correctAnswerIds) ? q.correctAnswerIds.map((v: any) => String(v)) : [];
        const isMultiple = String(q?.questionType) === 'multiple-choice' || q?.choicesConfig?.isMultipleAnswer === true;
        return {
          id,
