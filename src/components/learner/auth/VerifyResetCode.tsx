  ...
  useEffect(() => {
    ...
    if (error && 'data' in error) {
      toast({
        ...
        description: (error as any).data.message || 'Invalid code',
      });
            ...
            <div className="flex justify-center gap-4 mb-6">
              {Object.keys(verifyNumber).map((key, index) => (
                <input
                  ...
                  className={`w-14 h-14 border-2 text-black text-center text-lg font-semibold rounded-md outline-none transition ${invalidError ? 'border-red-500' : 'border-gray-400'} focus:ring-2 focus:ring-blue-500`}
                />
              ))}
            </div>
            <button
